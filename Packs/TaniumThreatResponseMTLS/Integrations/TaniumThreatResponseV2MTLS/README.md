> **UNOFFICIAL FORK (mTLS)** of the official *Tanium Threat Response v2* integration (Tanium Threat Response pack 2.2.32), patched to present a client certificate (mutual TLS) taken from the **Certificate** field of the credential. Temporary, until the official integration supports client certificates. See the integration Help (?) for the configuration.
>
> - The commands, arguments and outputs are the same as in the official integration, so playbooks work with either one. When both integrations have an enabled instance, a command runs on both unless the playbook task sets **Using**: disable the official instance, or set **Using** to the `Tanium Threat Response v2 mTLS` instance.
> - The rest of this page is the documentation of the official integration.
