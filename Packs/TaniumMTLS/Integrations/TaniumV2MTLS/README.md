> **UNOFFICIAL FORK (mTLS)** of the official *Tanium v2* integration (Tanium pack 1.0.44), patched to present a client certificate (mutual TLS) taken from the **Certificate** field of the credential. Temporary, until the official integration supports client certificates. See the integration Help (?) for the configuration.
>
> - The commands, arguments and outputs are the same as in the official integration, so playbooks work with either one. When both integrations have an enabled instance, a command runs on both unless the playbook task sets **Using**: disable the official instance, or set **Using** to the `Tanium v2 mTLS` instance.
